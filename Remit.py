...
with tab3:
    ...
    if final_df is None:
        ...
        remittance_file = st.file_uploader("📤 Upload Remittance File (with PO, Paid Amounts, etc.)", type=["xlsx"], key="remittance_file")
        ...
        if remittance_file:
            try:
                ...
                if not all(col in remit_df.columns for col in required_remit_cols):
                    ...
                    remit_df['PO'] = remit_df['PO'].astype(str).str.strip()
                    final_df['PO'] = final_df['PO'].astype(str).str.strip()

                    merged_remit_df = pd.merge(
                        final_df,
                        remit_df[required_remit_cols],
                        on='PO',
                        how='left'
                    )

                    st.success("✅ Remittance columns successfully merged into processed data!")
                    st.dataframe(merged_remit_df)

                    output_remit = io.BytesIO()
                    with pd.ExcelWriter(output_remit, engine='xlsxwriter') as writer:
                        merged_remit_df.to_excel(writer, index=False, sheet_name="Remittance_Merged")
                    output_remit.seek(0)

                    file_name_remit = (
                        st.session_state.get("final_output_file_name", "Remittance_Merged_Output.xlsx")
                        .replace(".xlsx", "_With_Remittance.xlsx")
                    )

                    st.download_button(
                        label="📥 Download File with Remittance Columns",
                        data=output_remit.getvalue(),
                        file_name=file_name_remit,
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                    )
            except Exception as e:
