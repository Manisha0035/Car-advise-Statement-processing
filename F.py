...
if statement_file and estimates_file:
    ...
    # Perform initial merge with indicator
    merged_df = pd.merge(
        statement_df, estimates_df, how='left',
        on=merge_key, indicator=True
    )

    # Match status from merge indicator
    ...
    if unmatched > 0:
        ...
        if query_result_file:
            ...
            # Merge unmatched with query result with indicator
            ...
            # Match status from _merge
            final_merged_unmatched['Match Status'] = final_merged_unmatched['_merge'].map({
                'both': 'Matched with Query Result',
                'left_only': 'Unmatched with Estimates and Query Result'
            })
            final_merged_unmatched.drop(columns=['_merge'], inplace=True)
