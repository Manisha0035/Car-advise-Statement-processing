...
with tab1:
    ...
    if statement_file and estimates_file:
        ...
        if unmatched_df.shape[0] > 0 and rebate_enrichment_df is not None:
            ...
            st.session_state["final_output_file_name"] = file_name  # Save filename to session state
            st.session_state["final_output_df"] = final_output  # Save df to session state for use in Tab 2 if needed
...
with tab2:
    ...
    # Use final output dataframe from Tab 1 if available
        ...
        if non_ai_file:
            try:
                ...
                if 'PO' not in df_non_ai.columns or 'PO' not in final_df.columns:
                    ...
                    # Use the same filename from Tab 1
